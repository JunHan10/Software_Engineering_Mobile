...
class ProfileController extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      print(
        'ProfileController: User data loaded: ${userData['user']?.firstName} ${userData['user']?.lastName}',
      );
      ...
      if (user?.id != null) {
        try {
          ...
          userAssets = assetsData
              ...
              .toList();
          print(
            'ProfileController: Loaded ${userAssets.length} assets for user',
          );
        } catch (e) {
      ...
      print(
        'ProfileController: Updating state with user: ${userData['user']?.email}',
      );
      _updateState(
        _state.copyWith(
          user: userData['user'],
  ...
  Future<void> pickProfileImage() async {
    ...
    try {
      ...
      if (image != null) {
        _updateState(
          _state.copyWith(profileImage: image, isPickingImage: false),
        );
      } else {
  ...
  Future<void> pickGalleryImages() async {
    try {
      ...
      if (images.isNotEmpty) {
        final updatedGallery = List<File>.from(_state.galleryImages)
          ..addAll(images.cast<File>());
        _updateState(_state.copyWith(galleryImages: updatedGallery));
      }
    } catch (e) {
      _updateState(
        _state.copyWith(errorMessage: 'Failed to pick gallery images: $e'),
      );
    }
  }
  ...
  Future<void> depositHippoBucks(String amountText) async {
    ...
    try {
      ...
      _updateState(
        _state.copyWith(hippoBalanceCents: newBalance, clearError: true),
      );
    } catch (e) {
      _updateState(_state.copyWith(errorMessage: 'Failed to deposit: $e'));
    }
  }
  ...
  Future<void> withdrawHippoBucks(String amountText) async {
    ...
    try {
      ...
      _updateState(_state.copyWith(errorMessage: 'Failed to withdraw: $e'));
    }
  }
  ...
  Future<void> updateProfile(String firstName, String email) async {
    ...
    try {
      ...
      if (updatedUser != null) {
        _updateState(_state.copyWith(user: updatedUser, clearError: true));
      }
    } catch (e) {
      _updateState(
        _state.copyWith(errorMessage: 'Failed to update profile: $e'),
      );
    }
  }
